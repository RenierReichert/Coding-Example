...
public class ExperimentLoggerNew : MonoBehaviour
{
    private static string currentParticipantPath;
    private static int participantID;
    //string hexID;       // = participantID.ToString("x").PadLeft(8,'0');
    ...
    public List<string> videoListBase;
    private static List<string> videoListDynamic;
    ...
    private void Awake()
    {
        ...
        //{
            //participantID = ((long)(UnityEngine.Random.value * 10000000000000000));
            participantID = UnityEngine.Random.Range(Int32.MinValue, Int32.MaxValue);

            // Seed our random generator with the participant ID, to be used when picking videos.
            UnityEngine.Random.InitState(participantID);

            //hexID = participantID.ToString("x").PadLeft(8,'0');
            //Debug.Log("hexID:"+hexID);
            //Debug.Log("intID:"+Convert.ToInt32(hexID, 16));
        //}

        videoListDynamic = new List<string>(videoListBase);

        /*string videoOrder = null;
        while (videoListDynamic.Count != 0)
        {
            videoOrder += GetNextRandomVideo();
        }
        Debug.Log("Video order:" + videoOrder);*/
    ...
    {
        if (GetComponent<ExperimentToggle>().experimenting)
        {
            answerGiven = false;
    ...
    public static int GetNextRandomValue()
    {
        return (int)UnityEngine.Random.value;
    }

    public static string GetNextRandomVideo()
    {
        if (videoListDynamic.Count != 0)
        {
            int videoIndex = UnityEngine.Random.Range(0, videoListDynamic.Count);
            string videoName = videoListDynamic[videoIndex];
            videoListDynamic.RemoveAt(videoIndex);
            return videoName;
        }
        else
        {
            //We're done, this was the final video! Return -1 as signal?
            return null;
        }
    }
}
